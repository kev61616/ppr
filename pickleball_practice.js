// Practice module functionality for PPR Level 2 Certification
document.addEventListener('DOMContentLoaded', function() {
  // Practice module data - questions about pickleball techniques
  ...
  function showQuestion(index) {
    ...
    const progressPercent = ((index + 1) / practiceQuestions.length) * 100;
    progressBar.style.width = `${progressPercent}%`;
    
    const questionIndicator = document.getElementById('question-indicator');
    questionIndicator.textContent = `Question ${index + 1} of ${practiceQuestions.length}`;
    ...
    question.options.forEach((option, optionIndex) => {
      ...
      optionElement.addEventListener('click', function() {
        selectOption(this, option.correct);
      });
  ...
  function selectOption(optionElement, isCorrect) {
    ...
    userAnswers[currentQuestionIndex] = {
      questionId: practiceQuestions[currentQuestionIndex].id,
    ...
    if (isCorrect) {
      ...
      options.forEach((opt, index) => {
        if (practiceQuestions[currentQuestionIndex].options[index].correct) {
    ...
    feedbackText.textContent = practiceQuestions[currentQuestionIndex].explanation;
  ...
  function showResults() {
    ...
    scoreElement.textContent = `${score}/${practiceQuestions.length} (${percentage}%)`;
